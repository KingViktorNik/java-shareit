...
class BookingServiceTest {
    ...
    void addBookingNullUserId() {
        ...
        final ValidationException exception = assertThrows(
                ...
                () -> bookingService.addBooking(null, null)
        );
    ...
    @Test
    void addBookingRentalDateInThePast() {
        // given
        bookingInsertDto.setEnd(bookingInsertDto.getStart().minusHours(1)); // дата аренды в прошлом
        when(itemRepository.findById(anyLong()))
                .thenReturn(Optional.of(item));
        ...
        // then
        Assertions.assertEquals(bookingInsertDto.getEnd() + " It can't be in the past", exception.getMessage());
    }
    ...
    @Test
    void getAllByUserOwnerInvalidStatus() {
        // given
        when(userRepository.findById(anyLong()))
                .thenReturn(Optional.of(user));
        ...
        final ValidationException exception = assertThrows(
                ValidationException.class,
                () -> bookingService.getAllByUserOwner(user.getId(), "status", 0, 10)
        );

        // then
        Assertions.assertEquals("Unknown state: status", exception.getMessage());
    }
    ...
    void getAllByUserOwnerAllHappy() {
        ...
        // when
        bookingService.getAllByUserOwner(user.getId(), StatusBooking.ALL.toString(), 0, 10);
    ...
    void getAllByUserOwnerCurrentHappy() {
        ...
        when(bookingRepository.getBookingOwnerCurrent(any(PageRequest.class),anyLong(), any(LocalDateTime.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllByUserOwner(user.getId(), StatusBooking.CURRENT.toString(), 0, 10);
    ...
    void getAllByUserOwnerPastHappy() {
        ...
        when(bookingRepository.findAllByItem_OwnerIdAndEndDateBefore(any(PageRequest.class),anyLong(), any(LocalDateTime.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllByUserOwner(user.getId(), StatusBooking.PAST.toString(), 0, 10);
    ...
    void getAllByUserOwnerFutureHappy() {
        ...
        when(bookingRepository.findAllByItem_OwnerIdAndStartDateAfter(any(PageRequest.class),anyLong(), any(LocalDateTime.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllByUserOwner(user.getId(), StatusBooking.FUTURE.toString(), 0, 10);
    ...
    void getAllByUserOwnerWaitingHappy() {
        ...
        when(bookingRepository.getBookingWaitingByOwner(any(PageRequest.class),anyLong()))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllByUserOwner(user.getId(), StatusBooking.WAITING.toString(), 0, 10);
    ...
    void getAllByUserOwnerRejectedHappy() {
        ...
        when(bookingRepository.findAllByStatusAndItem_OwnerId(any(PageRequest.class), any(StatusBooking.class), anyLong()))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllByUserOwner(user.getId(), StatusBooking.REJECTED.toString(), 0, 10);
    ...
    @Test
    void getAllBookingsByUserInvalidStatus() {
        // given
        when(userRepository.findById(anyLong()))
                .thenReturn(Optional.of(user));
        ...
        final ValidationException exception = assertThrows(
                ValidationException.class,
                () -> bookingService.getAllBookingsByUser(user.getId(), "status", 0, 10)
        );
    ...
    void getAllBookingsByUserAllHappy() {
        ...
        when(bookingRepository.findAllByBooker_Id(any(PageRequest.class),anyLong()))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllBookingsByUser(user.getId(), StatusBooking.ALL.toString(), 0, 10);
    ...
    void getAllBookingsByUserCurrentHappy() {
        ...
        when(bookingRepository.getBookingCurrent(any(PageRequest.class),anyLong(), any(LocalDateTime.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllBookingsByUser(user.getId(), StatusBooking.CURRENT.toString(), 0, 10);
    ...
    void getAllBookingsByUserPastHappy() {
        ...
        when(bookingRepository.findAllByBooker_IdAndEndDateBefore(any(PageRequest.class),anyLong(), any(LocalDateTime.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllBookingsByUser(user.getId(), StatusBooking.PAST.toString(), 0, 10);
    ...
    void getAllBookingsByUserFutureHappy() {
        ...
        when(bookingRepository.findAllByBooker_IdAndStartDateIsAfter(any(PageRequest.class),anyLong(), any(LocalDateTime.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllBookingsByUser(user.getId(), StatusBooking.FUTURE.toString(), 0, 10);
    ...
    void getAllBookingsByUserWaitingHappy() {
        ...
        when(bookingRepository.getBookingStatusByUser(any(PageRequest.class),anyLong(), any(StatusBooking.class)))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllBookingsByUser(user.getId(), StatusBooking.WAITING.toString(), 0, 10);
    ...
    void getAllBookingsByUserRejectedHappy() {
        ...
        when(bookingRepository.getBookingRejected(any(PageRequest.class), anyLong()))
                .thenReturn(Page.empty());
        // when
        bookingService.getAllBookingsByUser(user.getId(), StatusBooking.REJECTED.toString(), 0, 10);
